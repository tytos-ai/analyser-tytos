...
use pnl_core::{NewFinancialEvent, NewEventType};
...
impl ZerionClient {
    pub fn new(
        ...
        headers.insert("Authorization", auth_header.parse().map_err(|e| {
            ZerionError::Config(format!("Invalid auth header: {}", e))
        })?);
    ...
    pub async fn get_wallet_transactions(
        ...
        info!("🔄 Starting unlimited transaction fetch for wallet: {}", wallet_address);
        info!("🎯 Filters: page_size={}, chain={}, trash={}, types={}",
        ...
        while let Some(url) = next_url.take() {
            ...
            if !response.status().is_success() {
                ...
                error!("❌ Zerion API error {} on page {}: {}", status, page_num, text);
            ...
            debug!("📄 Page {}: Response size: {} bytes", page_num, response_text.len());
            ...
            if zerion_response.data.is_empty() {
                info!("📄 Page {}: No more transactions, stopping pagination", page_num);
                break;
            }
            ...
            info!("📄 Page {}: Fetched {} transactions in {}ms, has_next: {}",
                  page_num, zerion_response.data.len(), page_elapsed.as_millis(), has_next);
        ...
        let avg_per_page = if page_num > 1 { total_elapsed.as_millis() / (page_num - 1) as u128 } else { 0 };

        info!("📊 Pagination Summary: {} pages, {} total transactions in {}ms",
              page_num - 1, all_transactions.len(), total_elapsed.as_millis());
        info!("⏱️ Performance: avg {}ms per page, {} transactions/second",
              avg_per_page,
              if total_elapsed.as_secs() > 0 { all_transactions.len() as u64 / total_elapsed.as_secs() } else { 0 });
    ...
    pub async fn get_wallet_transactions_with_limit(
        ...
        info!("🔄 Starting limited transaction fetch for wallet: {} (limit: {})", wallet_address, limit);
        info!("🎯 Filters: page_size={}, chain={}, trash={}, types={}",
              self.page_size, self.chain_ids, self.trash_filter, self.operation_types);

        while all_transactions.len() < limit {
            let Some(url) = next_url.take() else {
                info!("📄 No more pages available, stopping at {} transactions", all_transactions.len());
                break;
            };
            ...
            info!("📄 Page {}: Fetching {} more transactions ({}/{})",
                  page_num, remaining_needed, all_transactions.len(), limit);
            debug!("🌐 URL: {}", url);
            ...
            info!("📄 Page {}: Took {} of {} transactions in {}ms, has_next: {}",
                  page_num, to_take, available_count, page_elapsed.as_millis(), has_next);

            let progress_percentage = (all_transactions.len() as f64 / limit as f64 * 100.0) as u32;
            info!("🎯 Progress: {}/{} transactions ({}% complete)",
                  all_transactions.len(), limit, progress_percentage);
    ...
    pub fn convert_to_financial_events(
        ...
        info!("🔄 Converting {} transactions to financial events for wallet: {}",
              transactions.len(), wallet_address);

        for (tx_index, tx) in transactions.iter().enumerate() {
            debug!("🔍 Processing transaction {}/{}: {} (type: {})",
                   tx_index + 1, transactions.len(), tx.id, tx.attributes.operation_type);
            ...
            match tx.attributes.operation_type.as_str() {
                "trade" | "send" => {
                    ...
                    debug!("💱 Transaction {}: {} transfers to process", tx.id, transfer_count);

                    let mut tx_events = 0u32;
                    for (transfer_index, transfer) in tx.attributes.transfers.iter().enumerate() {
                        debug!("🔄 Processing transfer {}/{} in tx {}: {} {} (direction: {})",
                               transfer_index + 1, transfer_count, tx.id,
                               transfer.quantity.numeric, transfer.fungible_info.symbol,
                               transfer.direction);

                        match self.convert_transfer_to_event(tx, transfer, wallet_address) {
                            Some(event) => {
                                debug!("✅ Created {} event: {} {} @ {} USD = {} USD",
                                       if event.event_type == NewEventType::Buy { "BUY" } else { "SELL" },
                                       event.quantity, event.token_symbol,
                                       event.usd_price_per_token, event.usd_value);
                                events.push(event);
                                tx_events += 1;
                            }
                            None => {
                    ...
                    debug!("📊 Transaction {}: {} transfers → {} events", tx.id, transfer_count, tx_events);
                }
                _ => {
                    debug!("⏭️ Skipping transaction {} (type: {})", tx.id, tx.attributes.operation_type);
                    skipped_count += 1;
                }
            }
        }
        ...
        info!("✅ Conversion complete: {} transactions → {} financial events in {}ms",
              transactions.len(), events.len(), total_elapsed.as_millis());
        info!("📊 Processing stats: {} processed, {} skipped, {} errors",
              processed_count, skipped_count, error_count);
        info!("⏱️ Processing rate: {} transactions/second", processing_rate);

        if error_count > 0 {
            warn!("⚠️ {} transfers had data quality issues and were skipped", error_count);
        }
    ...
    fn convert_transfer_to_event(
        ...
        let amount = match Decimal::from_str_exact(&transfer.quantity.numeric) {
            Ok(amt) => amt,
            Err(e) => {
                warn!("Failed to parse amount '{}': {}", transfer.quantity.numeric, e);
                return None;
            }
        };
        ...
        let event_type = match tx.attributes.operation_type.as_str() {
            ...
            _ => {
                warn!("Unexpected operation type: {}", tx.attributes.operation_type);
                return None;
            }
        };

        // Extract Solana contract address
        let mint_address = transfer.fungible_info.implementations
            .iter()
        ...
        let (usd_price_per_token, usd_value) = match (transfer.price, transfer.value) {
            (Some(price), Some(value)) => {
                ...
                debug!("✅ Using direct price data: price=${:.6}, value=${:.6} for {}",
            ...
            (Some(price), None) => {
                ...
                info!("🔄 Inferring value from price: price=${:.6} * quantity={} = ${:.6} for {}",
            ...
            (None, Some(value)) => {
                ...
                if quantity_f64 > 0.0 {
                    ...
                    info!("🔄 Inferring price from value: value=${:.6} / quantity={} = ${:.6} for {}",
                    ...
                    warn!("⚠️  Cannot calculate price: zero quantity for {}", transfer.fungible_info.symbol);
                    (Decimal::ZERO, Decimal::from_f64_retain(value).unwrap_or(Decimal::ZERO))
                }
            }
            (None, None) => {
                // Both price and value are null - skip this transaction
                warn!("⚠️  Skipping transaction: both price and value are null for {} ({})",
...
mod tests {
    ...
    async fn test_real_api_call() {
        ...
        let result = client.get_wallet_transactions_with_limit(wallet_address, "usd", 10).await;

        match result {
            Ok(transactions) => {
                println!("Successfully fetched {} transactions", transactions.len());
                for tx in &transactions {
                    println!("Transaction: {} - Type: {} - Transfers: {}",
                        tx.attributes.hash,
