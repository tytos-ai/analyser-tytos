...
async fn test_sol_equivalent_unit_fix() {
    ...
    let processed_swaps = ProcessedSwap::from_birdeye_transactions(&[mock_tx])
        .expect("Failed to process swap");
    
    assert_eq!(processed_swaps.len(), 1, "Should create exactly 1 processed swap");
...
async fn test_dual_event_system_token_to_token() {
    ...
    assert_eq!(events.len(), 2, "Token-to-token swap should create exactly 2 events");
    ...
    assert_eq!(sell_event.event_type, pnl_core::EventType::Sell, "First event should be SELL");
    assert_eq!(sell_event.token_mint, "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", "Should be selling USDC");
    assert_eq!(sell_event.token_amount, Decimal::from(1000), "Should sell 1000 USDC");
    // Verify SOL amount is approximately 6.67 SOL (allowing for precision differences)
    assert!((sell_event.sol_amount - Decimal::new(667, 2)).abs() < Decimal::new(1, 2), "Should use SOL equivalent (~6.67)");
    ...
    assert_eq!(buy_event.event_type, pnl_core::EventType::Buy, "Second event should be BUY");
    assert_eq!(buy_event.token_mint, "rndrizKT3MK1iimdxRdWabcF7Zg7AR5T4nud4EkHBof", "Should be buying RENDER");
    assert_eq!(buy_event.token_amount, Decimal::from(50), "Should buy 50 RENDER");
    // Verify SOL amount is approximately 6.67 SOL (allowing for precision differences)  
    assert!((buy_event.sol_amount - Decimal::new(667, 2)).abs() < Decimal::new(1, 2), "Should use SOL equivalent (~6.67)");
    
    // Both events should have same SOL equivalent (conservation of value)
    assert_eq!(sell_event.sol_amount, buy_event.sol_amount, "Both events should have same SOL equivalent");
    
    // Verify metadata contains swap type information
    assert!(sell_event.metadata.extra.contains_key("swap_type"), "SELL event should have swap_type metadata");
    assert!(buy_event.metadata.extra.contains_key("swap_type"), "BUY event should have swap_type metadata");
    assert_eq!(sell_event.metadata.extra.get("swap_type").unwrap(), "token_to_token_sell");
    assert_eq!(buy_event.metadata.extra.get("swap_type").unwrap(), "token_to_token_buy");
    
    // Verify counterpart token references
    assert_eq!(sell_event.metadata.extra.get("counterpart_token").unwrap(), &buy_event.token_mint);
    assert_eq!(buy_event.metadata.extra.get("counterpart_token").unwrap(), &sell_event.token_mint);
...
async fn test_sol_to_token_single_event() {
    ...
    assert_eq!(events.len(), 1, "SOL → Token swap should create exactly 1 event");
    ...
    assert_eq!(event.event_type, pnl_core::EventType::Buy, "Should be BUY event");
    assert_eq!(event.token_mint, "BNso1VUJnh4zcfpZa6986Ea66P6TCp59hvtNJ8b1X85", "Should be buying BNSOL");
    assert_eq!(event.token_amount, Decimal::from(950), "Should buy 950 BNSOL");
    assert_eq!(event.sol_amount, Decimal::from(1000), "Should spend 1000 SOL");
...
async fn test_fifo_accounting_with_dual_events() {
    ...
    let token_to_token_swap = ProcessedSwap::from_birdeye_transactions(&[create_mock_token_to_token_transaction()]).unwrap();
    ...
    for (i, event) in all_events.iter().enumerate() {
        println!("  Event {}: {:?} {} {} (SOL: {})", 
                i + 1, event.event_type, event.token_amount, 
                &event.token_mint[..8], event.sol_amount);
    }
    ...
    assert_eq!(all_events[0].token_mint, "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"); // USDC
    
    // Event 2: Sell USDC (from token-to-token swap)
    assert_eq!(all_events[1].event_type, pnl_core::EventType::Sell);
    assert_eq!(all_events[1].token_mint, "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"); // USDC
    
    // Event 3: Buy RENDER (from token-to-token swap)
    assert_eq!(all_events[2].event_type, pnl_core::EventType::Buy);
    assert_eq!(all_events[2].token_mint, "rndrizKT3MK1iimdxRdWabcF7Zg7AR5T4nud4EkHBof"); // RENDER
