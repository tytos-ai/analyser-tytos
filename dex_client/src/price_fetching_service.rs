...
mod tests {
    ...
    fn create_test_config() -> PriceFetchingConfig {
        PriceFetchingConfig {
            primary_source: "jupiter".to_string(),
            fallback_enabled: true,
            fallback_source: "birdeye".to_string(),
    ...
    fn create_test_birdeye_config() -> BirdEyeConfig {
        BirdEyeConfig {
    ...
    #[test]
    fn test_price_fetching_service_creation() {
        let config = create_test_config();
        let birdeye_config = create_test_birdeye_config();
        let service = PriceFetchingService::new(config, Some(birdeye_config)).unwrap();
        assert!(service.birdeye_client.is_some());
    }

    #[test]
    fn test_price_fetching_service_creation_without_birdeye() {
        let config = create_test_config();
        let service = PriceFetchingService::new(config, None).unwrap();
        assert!(service.birdeye_client.is_none());
    }

    #[test]
    fn test_invalid_primary_source() {
        let mut config = create_test_config();
        config.primary_source = "invalid".to_string();
        
        // Service creation should succeed, but runtime calls will fail
        let service = PriceFetchingService::new(config, None).unwrap();
        assert!(service.birdeye_client.is_none());
    }
}