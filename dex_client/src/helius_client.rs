...
mod tests {
    ...
    fn create_test_config() -> HeliusConfig {
        HeliusConfig {
    ...
    fn test_wallet_address_validation() {
        ...
        assert!(client.validate_wallet_address("So11111111111111111111111111111111111111112").is_ok());
        
        // Invalid - empty
        assert!(client.validate_wallet_address("").is_err());
        
        // Invalid - too short
        assert!(client.validate_wallet_address("short").is_err());
    }

    #[test]
    fn test_build_transactions_url() {
        let config = create_test_config();
        let client = HeliusClient::new(config).unwrap();
        
        let wallet = "So11111111111111111111111111111111111111112";
        let url = client.build_transactions_url(wallet, None, 100);
        
        assert!(url.contains("addresses/So11111111111111111111111111111111111111112/transactions"));
        assert!(url.contains("type=SWAP"));
        assert!(url.contains("limit=100"));
        assert!(url.contains("api-key=test_key"));
    }

    #[test]
    fn test_build_transactions_url_with_before() {
        let config = create_test_config();
        let client = HeliusClient::new(config).unwrap();
        
        let wallet = "So11111111111111111111111111111111111111112";
        let before_sig = "test_signature";
        let url = client.build_transactions_url(wallet, Some(before_sig), 50);
        
        assert!(url.contains("before=test_signature"));
        assert!(url.contains("limit=50"));
    }

    #[tokio::test]
    async fn test_fetch_wallet_transactions_disabled() {
        let mut config = create_test_config();
        config.enabled = false;
        let client = HeliusClient::new(config).unwrap();
        
        let wallet = "So11111111111111111111111111111111111111112";
        let result = client.fetch_wallet_transactions(wallet, Some(10), None).await;
        
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), HeliusError::ConfigError(_)));
    }

    #[tokio::test]
    async fn test_fetch_wallet_transactions_invalid_address() {
        let config = create_test_config();
        let client = HeliusClient::new(config).unwrap();
        
        let result = client.fetch_wallet_transactions("invalid", Some(10), None).await;
        
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), HeliusError::InvalidWalletAddress(_)));
    }

    #[test]
    fn test_extract_token_balance_changes() {
        let config = create_test_config();
        let client = HeliusClient::new(config).unwrap();
        ...
        let test_transaction = HeliusTransaction {
            ...
            account_data: vec![HeliusAccountData {
                ...
                native_balance_change: -10000000, // -0.01 SOL
                token_balance_changes: vec![HeliusTokenBalanceChange {
                    ...
                    raw_token_amount: HeliusRawTokenAmount {
                        token_amount: "1000000".to_string(), // +1 token
                        decimals: 6,
                    },
                }],
            }],
        ...
        let changes = client.extract_token_balance_changes(&transactions, wallet);
        
        // Should have 2 changes: 1 token change + 1 SOL change
        assert_eq!(changes.len(), 2);
        
        // Check token change
        let token_change = changes.iter().find(|c| c.mint == "test_mint").unwrap();
        assert_eq!(token_change.operation, TokenOperation::Buy);
        assert_eq!(token_change.ui_amount, 1.0);
        assert_eq!(token_change.raw_amount, 1000000);
        
        // Check SOL change
        let sol_change = changes.iter().find(|c| c.mint == "So11111111111111111111111111111111111111112").unwrap();
        assert_eq!(sol_change.operation, TokenOperation::Sell);
        assert_eq!(sol_change.ui_amount, -0.01);
        assert_eq!(sol_change.raw_amount, -10000000);
    }

    #[test]
    fn test_convert_to_transaction_records() {
        let config = create_test_config();
        let client = HeliusClient::new(config).unwrap();
        ...
        let transactions = vec![test_transaction];
        let records = client.convert_to_transaction_records(&transactions, wallet);
        ...
        assert_eq!(record.source, "JUPITER");
        assert_eq!(record.fee, 5000);
        assert_eq!(record.sol_balance_change, -10000000);
        assert_eq!(record.token_changes.len(), 1);
        
        let token_change = &record.token_changes[0];
        assert_eq!(token_change.mint, "test_mint");
        assert_eq!(token_change.raw_amount, 1000000);
        assert_eq!(token_change.ui_amount, 1.0);
        assert_eq!(token_change.decimals, 6);
        assert_eq!(token_change.is_buy, true);
    }

    #[test]
    fn test_convert_transaction_no_changes() {
        let config = create_test_config();
        let client = HeliusClient::new(config).unwrap();
        ...
        let test_transaction = HeliusTransaction {
            ...
            account_data: vec![HeliusAccountData {
                account: wallet.to_string(),
                native_balance_change: 0, // No change
                token_balance_changes: vec![], // No token changes
            }],
            events: None,
            transaction_error: None,
            instructions: vec![],
        ...
        // Should be empty because no significant changes
