...
BEGIN
    ...
    LOOP
        ...
        LOOP
            -- Extract profit_percentage from JSON (already calculated by PnL engine)
            portfolio_data := wallet_rec.pjson;

            -- Get profit_percentage (may be string or number)
            BEGIN
                profit_percent := CASE
                    WHEN portfolio_data->>'profit_percentage' IS NOT NULL THEN
                        (portfolio_data->>'profit_percentage')::NUMERIC
                    ELSE 0
                END;
